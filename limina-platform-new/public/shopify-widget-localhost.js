// Limina Buy Orders Widget for Shopify (Localhost Version)
// This script connects to your local Limina development server

(function() {
  'use strict';

  const LIMINA_API_BASE = 'http://localhost:3000'; // Your local Limina server
    ...
    ">
      ...
      <div id="limina-show-form">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
          <span style="font-size: 14px; opacity: 0.9;">Current Price:</span>
      ...
      <div id="limina-success" style="display: none;">
        <div style="text-align: center;">
          ...
          <p style="margin: 0; opacity: 0.9;">
            We'll notify you if the price drops to £<span id="submitted-price"></span> or below.
          </p>
        </div>
      </div>
    </div>
  ...
  if (productForm) {
    ...
    // Fallback: insert after product info
    const productInfo = document.querySelector('.product__info') ||
                        document.querySelector('.product-single__meta') ||
                        document.querySelector('.product-details');
    if (productInfo) {
      productInfo.insertAdjacentHTML('afterend', widgetHTML);
    }
  }
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async function() {
      ...
      try {
        ...
        if (response.ok) {
          offerForm.style.display = 'none';
          document.getElementById('submitted-price').textContent = price;
          ...
          // Hide success message after 8 seconds and reset
          setTimeout(() => {
        ...
        alert('Error submitting offer. Please check your internet connection and try again.');
      } finally {
